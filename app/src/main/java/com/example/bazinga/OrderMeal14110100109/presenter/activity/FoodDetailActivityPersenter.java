...
import com.example.bazinga.OrderMeal14110100109.modle.activity.FoodDetailActivityModel;
...
public class FoodDetailActivityPersenter extends BasePresenter<IShowFoodView> {


    FoodDetailActivityModel foodDeatilActivityModel = new FoodDetailActivityModel();


    public void collectFood(int userId, int foodId) {
        if(foodDeatilActivityModel != null){
            foodDeatilActivityModel.setfoodId(foodId);
            foodDeatilActivityModel.setUserId(userId);
            foodDeatilActivityModel.collectFood(new CollectInterface() {
                @Override
    ...
    public void checkUserCollection(int userId, int shopId) {
        if(foodDeatilActivityModel != null){
            foodDeatilActivityModel.setfoodId(shopId);
            foodDeatilActivityModel.setUserId(userId);
            foodDeatilActivityModel.checkCollectShop(new CollectInterface() {
                @Override
                public void collectCompleted(Boolean result) {
                }
